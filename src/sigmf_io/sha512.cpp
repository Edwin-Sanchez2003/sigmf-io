...
void SHA512::parse_and_validate(const std::string& hex)
{
    if (hex.size() != kHexLength) {
        throw std::invalid_argument(
            "SHA512: '" + hex + "' is not " + std::to_string(kHexLength) +
...
bool SHA512::matches(std::span<const std::byte, kDigestBytes> digest) const
{
    return std::equal(bytes_.begin(), bytes_.end(), digest.begin());
}

bool SHA512::matches(std::span<const unsigned char, kDigestBytes> digest) const
{
