...
class SHA512 {
public:
    static constexpr std::size_t k_digest_bytes = 64;
    static constexpr std::size_t k_hex_length   = k_digest_bytes * 2;

    explicit SHA512(const std::string& hex);

    std::string to_string() const { return raw_; }
    std::string hex() const { return normalized_; }
    std::array<std::byte, k_digest_bytes> bytes() const { return bytes_; }
    ...
    std::span<const unsigned char, k_digest_bytes> as_bytes_view() const {
        return std::span<const unsigned char, k_digest_bytes>(
            reinterpret_cast<const unsigned char*>(bytes_.data()), k_digest_bytes);
    }
    ...
    bool matches(std::span<const std::byte, k_digest_bytes> digest) const;
    // Check a digest computed elsewhere (unsigned char form -- e.g. straight
    // out of OpenSSL's SHA512() or similar C-style crypto APIs).
    bool matches(std::span<const unsigned char, k_digest_bytes> digest) const;

private:
    std::string raw_;
    std::string normalized_;
    std::array<std::byte, k_digest_bytes> bytes_{};
