...
public:
    ...
    virtual std::expected<void, std::string> check_global(const Global& global) const override;
    virtual std::expected<void, std::string> check_global(const jsoncons::json& global) const override;
    virtual std::expected<void, std::string> check_capture(const Capture& capture) const override;
    virtual std::expected<void, std::string> check_capture(const jsoncons::json& capture) const override;
    virtual std::expected<void, std::string> check_annotation(const Annotation& annotation) const override;
    virtual std::expected<void, std::string> check_annotation(const jsoncons::json& annotation) const override;

    // validate a json object, assuming it is structured like a SigMF metadata file.
    virtual std::expected<void, std::string> check_metadata(const jsoncons::json& meta) const override;
